...
$protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "//" : "http://";
...
//echo '//www.google.com/recaptcha/api/siteverify?secret=' . $secretKey . "&response=" . $captcha;exit;
$response		 =  curlSubmit( $protocol . "www.google.com/recaptcha/api/siteverify", array( 'secret' => $secretKey, 'response' => $captcha, 'remoteip' => $ip ) ); //file_get_contents( "//www.google.com/recaptcha/api/siteverify?secret=" . $secretKey . "&response=" . $captcha . "&remoteip=" . $ip );
//$response        = file_get_contents( '//www.google.com/recaptcha/api/siteverify?secret=' . $secretKey . "&response=" . $captcha );
